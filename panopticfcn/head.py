...
class NeighborPrediction(nn.Module):
    def __init__(self, cfg):
        ...
        in_channel = cfg.MODEL.SEMANTIC_FPN.CONVS_DIM
        conv_dims = cfg.MODEL.NEIGHBOR_HEAD.CONV_DIMS
        out_channel = len(cfg.MODEL.NEIGHBOR_HEAD.DILATIONS) * 9
        norm = cfg.MODEL.NEIGHBOR_HEAD.NORM
        ...
        self.out_conv = Conv2d(conv_dims, out_channel, kernel_size=1).to(device)
...
class SemanticPrediction(nn.Module):
    def __init__(self, cfg):
        ...
        device = torch.device(cfg.MODEL.DEVICE)
        in_channel = cfg.MODEL.SEMANTIC_FPN.CONVS_DIM
        conv_dims = cfg.MODEL.SEMANTIC_HEAD.CONV_DIMS
        out_channel = cfg.MODEL.SEMANTIC_HEAD.NUM_CLASSES
        norm = cfg.MODEL.SEMANTIC_HEAD.NORM
...
def build_semantic_head(cfg, input_shape=None):
    return SemanticPrediction(cfg)

def build_neighbor_head(cfg, input_shape=None):
    return NeighborPrediction(cfg)
