...
from .visualize import Visualize, UseDetectronVisualize
__all__ = ["PanopticFCN"]
...
class PanopticFCN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.visualize = Visualize(train_dataset='coco_2017_train_panoptic_separated')
        self.detectron_visualize = UseDetectronVisualize()
        self.save_dir = '/home/lab/ssd1/DynamicConv/Code/detectron2/projects/GuidedPAKAP345PanopticFCN/visualize/D1248_neipredconv3ks1'
        self.iter = 0
    ...
    def forward(self, batched_inputs):
        ...
        * Other information that's included in the original dicts, such as:
        ...
        features, pred_semantics, pred_neighbors = self.backbone(images.tensor)
        pred_semantics = pred_semantics[::-1]  # [p3,p4,p5]
        pred_neighbors = pred_neighbors[::-1]  # [p3,p4,p5]
        ...
        if self.training:
            ...
            #visualize
            example = '000000386927' in [x['file_name'].split('/')[-1].split('.')[0] for x in batched_inputs]
            if example: print("find example")
            if self.iter % 5000 == 0 or example:
                self.do_visualize(batched_inputs, gt_dict, pred_semantics, pred_neighbors, pred_regions[0])

            self.iter +=1
            return self.losses(pred_centers, pred_regions, pred_weights, encode_feat, pred_semantics, pred_neighbors, gt_dict)

        else:
            return self.inference(batched_inputs, images, pred_centers, pred_regions, pred_weights, encode_feat)

    def do_visualize(self, batched_inputs, gt_dict, pred_semantics, pred_neighbors, pred_region):
        file_names = []
        gt_fb_semantics = gt_dict['gt_fb_semantics']
        gt_fb_neighbors = gt_dict['gt_fb_neighbors']

        for ii, x in enumerate(batched_inputs):
            file_name = x['file_name'].split('/')[-1].split('.')[0]
            file_name = 'iter' + str(self.iter) + '_' + file_name
            file_names.append(file_name)
            self.visualize.show_image(file_name, x["image"], self.save_dir)

            img_shape = x["image"].shape[-2:]
            pd_reg = pred_region[ii].unsqueeze(0)
            pd_reg = F.interpolate(pd_reg, scale_factor=8, mode='bilinear', align_corners=False)[0, :,:img_shape[0], :img_shape[1]]
            pd_reg = pd_reg.argmax(dim=0)
            self.detectron_visualize.show_stuff(x["image"], pd_reg, file_name, self.save_dir)

            self.visualize.show_semantic(file_name+ '_p3', gt_fb_semantics[0][ii],
                                         pred_semantics[0][ii],
                                         self.save_dir)
            self.visualize.show_neighbor(file_name+ '_p3', gt_fb_neighbors[0][ii],
                                         pred_neighbors[0][ii],
                                         self.save_dir)
    ...
    def losses(self, pred_centers, pred_regions, pred_weights, encode_feat, pred_semantics, pred_neighbors, gt_dict):
        """
        ...
        Args:
            ...
            pred_semantics: prediction for semantic in [p5,p4,p3]
            pred_neighbors: prediction for neighbor in [p5,p4,p3]
        ...
        #for semantic and neighbor for GuidedPAKA
        loss_semantics, loss_neighbors = multi_apply(self.loss_fpn_single_level, pred_semantics, pred_neighbors,
                                                       gt_dict['gt_fb_semantics'], gt_dict['gt_fb_neighbors'])
        ...
        # fpn loss
        loss["loss_fpn_sem"] =  sum(loss_semantics) / max(feat_shape[0], 1)
        loss["loss_fpn_nei"] = sum(loss_neighbors)/ max(feat_shape[0], 1)

        return loss

    def loss_fpn_single_level(self, pred_sem, pred_nei, gt_sem, gt_nei):
        loss_sem = sigmoid_focal_loss(pred_sem, gt_sem,
                                        mode="semantic",
        ...
        for i in range(b):
            ...
            pd = pred_nei[i][mask]
            gt = gt_nei[i][mask]
            loss = sigmoid_focal_loss(pd, gt,
                                           alpha=-1,
                                           gamma=self.focal_loss_gamma,
                                           reduction="mean")
            loss_nei += loss

        # loss_nei = sigmoid_focal_loss(pred_nei, gt_nei,
        #                                 mode="neighbor",
        #                                 alpha=-1,
        #                                 gamma= self.focal_loss_gamma,
        #                                 reduction="sum")
        return loss_sem, loss_nei
